...
import { ArrowLeft, Archive, Loader2, Info } from 'lucide-react';
import FileSelector from '../../components/FileSelector';
import ResultScreen from './shared/ResultScreen';
import { LicenseStatus } from '../../types';

interface PdfToPdfaPagesProps {
  onBack: () => void;
  license: LicenseStatus;
}

export default function PdfToPdfaPages({ onBack }: PdfToPdfaPagesProps) {
  ...
  const handleExecute = async () => {
    ...
    setErrorMsg('');
    try {
      const outFilePath = `${outputPath}/PDFA_${Date.now()}.pdf`;
      const res = await window.electronAPI.convertPdfToPdfA(fileSelected, outFilePath);
      if (res.success) {
        setSuccessPath(res.outputPath);
      }
    } catch (err: any) {
      setErrorMsg(err.message || 'An error occurred during PDF/A conversion. Make sure Ghostscript is installed and configured in Settings > Advanced.');
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex items-center gap-3 border-b border-border pb-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-foreground">PDF to PDF/A</h2>
          <p className="text-[10px] text-primary font-bold uppercase tracking-wider">Archival Conversion</p>
        </div>
      </div>
        ...
        <div className="max-w-2xl mx-auto glass-panel border border-border rounded-2xl p-6 space-y-6">
          <div className="text-center p-4 bg-secondary/35 rounded-xl border border-border space-y-2">
            <h3 className="text-sm font-bold text-foreground flex items-center justify-center gap-2">
              <Archive className="w-4 h-4 text-primary animate-pulse" />
              ISO-Compliant Archival Format
            </h3>
            <p className="text-xs text-muted-foreground max-w-md mx-auto leading-relaxed">
              Convert your PDF to PDF/A-2, an ISO-standardized version designed for long-term document preservation. Fonts and color profiles are embedded.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 bg-primary/5 border border-primary/10 rounded-xl text-[10px] text-primary font-bold flex items-start gap-2">
              ...
              <span>Requires the Ghostscript binary. The app auto-detects standard install paths.</span>
            </div>
