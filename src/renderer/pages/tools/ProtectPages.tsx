...
interface ProtectPagesProps {
  onBack: () => void;
  license: LicenseStatus;
}

export default function ProtectPages({ onBack, license }: ProtectPagesProps) {
  ...
  const handleExecute = async () => {
    if (!password) {
      setErrorMsg('Please enter an encryption password.');
      return;
    }
    ...
    setErrorMsg('');
    try {
      const outFilePath = `${outputPath}/Protected_${Date.now()}.pdf`;
      const res = await window.electronAPI.protectPDF(fileSelected!, outFilePath, password);
      if (res.success) {
        setSuccessPath(res.outputPath);
      }
    } catch (err: any) {
      setErrorMsg(err.message || 'An error occurred during password encryption.');
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-3 border-b border-border pb-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-foreground">Protect PDF with Password</h2>
          <p className="text-[10px] text-primary font-bold uppercase tracking-wider">Local Encryption</p>
        </div>
      </div>
        ...
        <div className="max-w-2xl mx-auto glass-panel border border-border rounded-2xl p-6 space-y-6">
          <div className="text-center p-4 bg-secondary/35 rounded-xl border border-border space-y-2">
            ...
            <p className="text-xs text-muted-foreground max-w-md mx-auto leading-relaxed">
              We encrypt the document file using standard Ghostscript algorithms. The password will be required to open the PDF.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] text-muted-foreground font-extrabold uppercase tracking-wider">Set Password</label>
                <input
                  type="password"
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-muted-foreground font-extrabold uppercase tracking-wider">Confirm Password</label>
                <input
                  type="password"
