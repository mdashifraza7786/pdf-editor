...
import { ArrowLeft, FileOutput, Loader2, Check } from 'lucide-react';
import FileSelector from '../../components/FileSelector';
import usePdfRenderer from './shared/usePdfRenderer';
import ResultScreen from './shared/ResultScreen';
import { LicenseStatus } from '../../types';

interface ExtractPagesProps {
  onBack: () => void;
  license: LicenseStatus;
}

export default function ExtractPages({ onBack, license }: ExtractPagesProps) {
  ...
  const handleExecute = async () => {
    ...
    setErrorMsg('');
    try {
      const outFilePath = `${outputPath}/Pages_Extracted_${Date.now()}.pdf`;
      const sortedPagesToExtract = Array.from(selectedPages).sort((a, b) => a - b);
      const res = await window.electronAPI.extractPages(fileSelected!, sortedPagesToExtract, outFilePath);
      if (res.success) {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-3 border-b border-border pb-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-foreground">Extract PDF Pages</h2>
                ...
                {pages.map((dataUrl, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Overlays */}
                      {isSelected && (
                        <div className="absolute inset-0 bg-emerald-500/10 flex items-center justify-center transition-opacity">
                          <div className="p-2.5 rounded-full bg-emerald-600 text-white shadow">
                      ...
                      {isSelected && (
                        <span className="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-600 text-[10px] font-bold text-white shadow">
                          {pageNum}
                        </span>
                      )}
                    </div>
          ...
          <div className="space-y-6">
            <div className="glass-panel border border-border rounded-2xl p-5 space-y-4">
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <div className="flex justify-between py-1.5 border-b border-border">
                  <span className="text-muted-foreground font-medium">Pages to Keep</span>
                  <span className="font-bold text-emerald-600">{selectedPages.size}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-border">
                  <span className="text-muted-foreground font-medium">Excluded Pages</span>
                  <span className="font-bold text-foreground">{pages.length - selectedPages.size}</span>
                </div>
              </div>
